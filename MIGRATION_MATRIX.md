...
| Déploiement | Compose de développement; runtime production-like terminé localement en P11 (`infra/docker-compose.prodlike.yml`, Nginx non-root, backend non-root, 3 réseaux, secrets requis, profil `observability` optionnel), vérifié par un smoke test Playwright réel contre la stack démarrée | P11 pas encore fusionnée dans `main` (PR pas encore ouverte); staging en P15. |
