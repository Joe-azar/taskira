...
| Observabilité | Terminé P9 (corrélation/logs) et P10 localement (métriques) : `X-Request-Id`, MDC, `ProblemDetails.of(...)`, logs structurés JSON en `prod`; Actuator sur port de gestion isolé (9091, jamais publié à l'hôte), Prometheus (`v3.13.2`) target `UP` vérifié, Grafana (`13.1.3`) datasource + 2 dashboards provisionnés et vérifiés contre la stack réelle | P10 pas encore fusionnée dans `main` (PR pas encore ouverte). |
