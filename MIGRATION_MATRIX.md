...
| CI/CD | `main` protégée depuis P6 (PR + `CI Gate` obligatoires, force-push/suppression interdits); run GitHub vert sur `ci.yml`, `quality.yml`, `security.yml`, `codeql.yml` au commit `7058041` (fin P5) | Vérifier les mêmes workflows sur le HEAD de `feat/phase6-stack-upgrade` avant fusion; GHCR/staging/release restent en P15. |
