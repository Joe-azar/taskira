...
- La phase 7 est **terminée et fusionnée** dans `main` (PR #30). `/api/v1` versionne les six contrôleurs; profils Spring `dev`/`test`/`prod` avec différences réelles (logs, springdoc désactivé en `prod`, vérifié par test); erreurs migrées vers `ProblemDetail` RFC 7807/9457 (backend et ~25 sites frontend); `AuthService`/`UserService`/`DashboardService` couverts par `@Transactional`; verrouillage optimiste (`@Version`, Flyway `V7`) avec 409 sur conflit, prouvé contre PostgreSQL réel. 22 tests backend, 20 Vitest, 9/9 Playwright.
- La phase 8 est terminée sur `feat/phase8-session-security`, pas encore fusionnée dans `main` au moment de cette mise à jour. Authentification migrée du JWT stateless (`localStorage`) vers une session Spring Security cookie `TASKIRA_SESSION` (`HttpOnly`, `SameSite=Lax`, `Secure` uniquement en `prod`) avec CSRF double-soumission (`CookieCsrfTokenRepository` + `CsrfTokenRequestAttributeHandler` explicite + `CsrfCookieFilter`), menée en 4 incréments atomiques (session/CSRF en mode dual avec le JWT encore actif, migration frontend complète et vérifiée par E2E, retrait du JWT une fois le remplacement prouvé fonctionnel, bootstrap admin dev idempotent gaté au profil `dev`). Aucun jeton d'authentification en `localStorage`/`sessionStorage`. Voir [ADR-0006](docs/adr/0006-session-cookie-auth.md) et la section phase 8 du rapport cumulatif pour le détail complet, y compris les bugs réels trouvés et corrigés via E2E. 39 tests backend (20 rapides + 19 intégration), 25 Vitest, 9/9 Playwright.
- Les phases 9 à 20 sont planifiées et aucune technologie future ne doit être présentée comme installée.
...
- L'authentification utilise depuis la phase 8 une session Spring Security et un cookie `HttpOnly`/`SameSite=Lax` (`Secure` en `prod`) avec protection CSRF double-soumission ([ADR-0006](docs/adr/0006-session-cookie-auth.md)); le JWT et le stockage `localStorage`/`sessionStorage` de jeton d'authentification ont été retirés. Ne pas réintroduire de secret durable côté client.
