...
- La phase 3 est partielle : le workflow GitHub Actions #3 est vert sur le commit `6db6115` de la PR draft #1, y compris le lint frontend. Seule la protection de `main` reste à activer, faute d'authentification GitHub interactive sur ce poste.
- La phase 4 est terminée localement : SonarQube Community Build (Docker, éphémère) donne une Quality Gate `OK` (0 bug, 0 vulnérabilité, 0 hotspot, 24 code smells, couverture 13,0 %); Trivy (dépendances + images) et CodeQL sont configurés et exécutés localement. `.github/workflows/quality.yml`, `security.yml` et `codeql.yml` ainsi que `.github/dependabot.yml` sont créés mais non vérifiés sur un run GitHub distant, faute de la même authentification. Les CVE réelles détectées (21 backend, 8 frontend) restent ouvertes et sont un gap P6 documenté, pas un échec P4.
...
- La phase 6 est terminée : `main` est protégée (PR + `CI Gate` obligatoires, force-push et suppression interdits, pas de revue humaine obligatoire, admin non bloqué) et toute la migration a été menée sur `feat/phase6-stack-upgrade`. Backend : Spring Boot 4.1.0 (Spring Framework 7.0.8, Spring Security 7.1.0, Hibernate 7.4.1), Spring Modulith 2.1.0, Springdoc 3.1.0, MapStruct 1.6.3, Jackson 3 (`tools.jackson`, plus `com.fasterxml.jackson`). Frontend : Angular/CLI/CDK/Material 22.1.x, TypeScript 6.0.3, Node 24.19.0 LTS, npm 12.0.2. Base de données : PostgreSQL 18.6, nouveau point de montage `/var/lib/postgresql` (voir [ADR-0017](docs/adr/0017-postgresql-18-migration.md)), migration par sauvegarde/restauration réelle avec l'ancien volume conservé intact. 18 tests backend et 9 scénarios Playwright vérifiés verts après chaque sous-étape; deux bugs réels indépendants du contenu de la migration ont aussi été trouvés et corrigés en route (fins de ligne CRLF cassant l'entrypoint E2E, assertion Playwright fragile face à un rechargement applicatif). Voir `ENTERPRISE_MIGRATION_REPORT.md` pour le détail complet et ce qui reste (vérification CI distante, fusion vers `main`).
