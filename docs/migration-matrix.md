...
| 10 | Observabilité | Haute | Terminée localement | Actuator sur port de gestion isolé (`EndpointRequest.toAnyEndpoint().permitAll()`), Micrometer, Prometheus (`v3.13.2`) target `UP` vérifié via `/api/v1/targets`, Grafana (`13.1.3`) provisionné (datasource + 2 dashboards) et métriques techniques/métier vérifiées contre la stack réelle. 76 tests backend, 10/10 Playwright. PR pas encore ouverte. |
