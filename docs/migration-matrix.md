...
| 4 | SonarQube et scans | Haute | Terminée localement | Quality Gate SonarQube `OK` (0 bug, 0 vulnérabilité, 0 hotspot), Trivy dépendances/images et CodeQL exécutés et documentés localement; run GitHub distant non vérifié faute d'authentification GitHub. |
...
| 6 | Montées technologiques | Haute | Terminée sur `feat/phase6-stack-upgrade` | Java 21 conservé; Spring Boot 4.1.0/Spring 7.0.8, Angular 22.1.x, Node 24.19.0, Material 22.1.2 et PostgreSQL 18.6 passent la non-régression (18 tests backend, 9 scénarios Playwright). Branche protégée à fusionner vers `main` après vérification CI distante. |
