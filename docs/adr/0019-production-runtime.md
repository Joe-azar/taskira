...
Vérifié réellement, de bout en bout, par un smoke test Playwright avec un vrai navigateur contre la stack production-like démarrée pour de vrai (pas seulement par `curl`) : `uid=10001(taskira)`/`uid=101(nginx)` non-root confirmés, seul le port de `frontend` accessible depuis l'hôte, connectivité interne `frontend`→`backend` et `prometheus`→`backend:9091` fonctionnelle, inscription/connexion/navigation authentifiée réussies avec un cookie `TASKIRA_SESSION` réel (`httpOnly=true`, `sameSite=Lax`, `secure=false` — exactement le résultat attendu de la résolution du conflit Secure/HTTP ci-dessus).
