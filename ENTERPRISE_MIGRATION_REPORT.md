...
Les phases 0 à 13 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 13 (pièces jointes, Tika, stockage local) a été fusionnée dans `main` le 16 août 2026 (PR [#36](https://github.com/Joe-azar/taskira/pull/36), commit de fusion `0e535d2cd94e84fabaa3163859a33f8a48f4604f`) sur la branche dédiée `feat/phase13-attachments`, supprimée après fusion. Un bug réel supplémentaire a été trouvé uniquement par le run GitHub Actions de la PR (valeur par défaut de `app.attachments.storage-path` non écrivable par un utilisateur non privilégié, masquée localement par un conteneur Maven tournant en root) et corrigé avant fusion — détail complet dans la section dédiée ci-dessous. La phase 14 (exports Excel/PDF/QR, Spring Batch) est terminée localement sur la branche dédiée `feat/phase14-exports` (115 tests backend, 25 Vitest, 10/10 Playwright, plus un smoke test manuel réel contre la vraie stack de développement) mais pas encore fusionnée : PR à ouvrir/merger par le propriétaire du dépôt (`gh` non authentifié dans cet environnement). Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`, `feat/phase12-notifications`, `feat/phase13-attachments`, `feat/phase14-exports`), jamais directement sur `main`. Les phases 15 à 20 ne sont pas déclarées terminées.
...
Menée entièrement sur `feat/phase14-exports`, en trois commits vérifiés indépendamment (`921d73c` à `1bb8500`), précédée d'un ADR (`0022`) écrit avant tout code. Terminée localement, pas encore fusionnée : la PR reste à ouvrir/merger par le propriétaire du dépôt (`gh` non authentifié dans cet environnement, comme à chaque phase précédente).
