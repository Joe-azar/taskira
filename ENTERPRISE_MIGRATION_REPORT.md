...
Les phases 0 à 15 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 15 (registry GHCR, release, staging, rollback) a été fusionnée dans `main` le 22 août 2026 (PR [#46](https://github.com/Joe-azar/taskira/pull/46), commit de fusion `e2072bc6b2c8f3571d6b5db663b5e698bbaeeb14`) sur la branche dédiée `feat/phase15-registry-staging`, supprimée après fusion. Son workflow de release avait déjà été déclenché et vérifié réellement à deux reprises par un vrai tag `v0.1.0` avant même l'ouverture de la PR (deux bugs réels trouvés et corrigés au premier essai, second essai entièrement vert : images publiées sur GHCR, staging réellement déployé, 10/10 Playwright contre ce déploiement réel, [GitHub Release v0.1.0](https://github.com/Joe-azar/taskira/releases/tag/v0.1.0) publiée) — détail complet dans la section dédiée ci-dessous. La phase 16 (sauvegarde et restauration testée) a été fusionnée dans `main` le 22 août 2026 sur la branche dédiée `feat/phase16-backup-restore` (PR [#47](https://github.com/Joe-azar/taskira/pull/47), commit de fusion `3eacaaf`), supprimée après fusion. Un cycle complet sauvegarde/restauration avait déjà été vérifié réellement en local avant fusion, révélant et corrigeant deux bugs réels. Le tout premier run réel du nouveau workflow planifié sur GitHub Actions — impossible à pré-tester avant fusion, `workflow_dispatch` ne pouvant déclencher un fichier de workflow inexistant sur la branche par défaut — a lui-même révélé un troisième bug réel (capture `Write-Host` au lieu d'une vraie valeur de retour PowerShell), corrigé sur une branche de suivi dédiée `fix/backup-drill-real-capture` (PR [#48](https://github.com/Joe-azar/taskira/pull/48), commit de fusion `e03c51f`); un second run réel après ce correctif est passé intégralement au vert. Détail complet dans la section dédiée ci-dessous. Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`, `feat/phase12-notifications`, `feat/phase13-attachments`, `feat/phase14-exports`, `feat/phase15-registry-staging`, `feat/phase16-backup-restore`, `fix/backup-drill-real-capture`), jamais directement sur `main`. Les phases 17 à 20 ne sont pas déclarées terminées.
...
| 2026-08-16 | 12 — Notifications et Mailpit | **Terminée et fusionnée** | PR #35, commit de fusion `40e791ed30cdd5bb0bb84bbadd7be7a91924907a`, branche `feat/phase12-notifications` supprimée après fusion. 6 commits vérifiés indépendamment (`2016f11`..`318e4c7`) : ADR-0020, `TicketAssignedEvent`/`CommentCreatedEvent` (premiers événements métier réels du projet), `NotificationService` (`@TransactionalEventListener(AFTER_COMMIT)`, best-effort), Mailpit (`axllent/mailpit:v1.30.7`) dans `infra/docker-compose.yml`. 83 tests backend (38 rapides + 45 intégration, dont `NotificationWiringIT` contre un vrai conteneur Mailpit), 25 Vitest (inchangé). Un bug réel trouvé par la suite complète (indicateur de santé Actuator pour le courrier faisant échouer `/actuator/health`, corrigé). Vérifié en plus contre la vraie stack de développement : email réellement reçu dans Mailpit après une assignation via l'API réelle. Détail complet ci-dessous. | Aucun pour le critère de sortie de P12. |
| 2026-08-16 | 13 — Pièces jointes | **Terminée et fusionnée** | PR [#36](https://github.com/Joe-azar/taskira/pull/36), commit de fusion `0e535d2cd94e84fabaa3163859a33f8a48f4604f`, branche `feat/phase13-attachments` supprimée après fusion. 8 commits vérifiés indépendamment (`817d8fc`..`6104def`) : ADR-0021 (ADR-0009 promu Accepted), module `attachments` (`Attachment`, Flyway `V9`), port `DocumentStorage`/`LocalFileSystemStorage`, détection de type réel par Apache Tika avec liste blanche, clé de stockage UUID jamais dérivée du client, SHA-256, endpoints upload/téléchargement/suppression audités. 100 tests backend (51 rapides + 49 intégration, dont `AttachmentWiringIT`), 25 Vitest et 10/10 Playwright inchangés (P13 est backend uniquement). Quatre bugs réels trouvés et corrigés, tous revérifiés : SHA-256 d'un flux jamais lu par un mock, ordre de validation exposant une erreur trompeuse, répertoire de stockage non accessible en écriture au backend non-root de P11 (trouvé en démarrant la vraie stack), et une valeur par défaut de chemin de stockage non écrivable par un utilisateur non privilégié (trouvée uniquement par le vrai run GitHub Actions de la PR, masquée localement par un conteneur Maven tournant en root). CI Gate, Backend, SonarQube, CodeQL et Trivy tous vérifiés verts sur GitHub avant fusion. Détail complet ci-dessous. | Aucun pour le critère de sortie de P13. |
| 2026-08-16 | 14 — Exports | **Terminée et fusionnée** | PR [#37](https://github.com/Joe-azar/taskira/pull/37), commit de fusion `2f84d5ba39ab57ef65da868a026aab06be3fa646`, branche `feat/phase14-exports` supprimée après fusion. 4 commits vérifiés indépendamment (`921d73c`..`a8aa118`) : ADR-0022, export Excel synchrone par projet (POI), export PDF synchrone par ticket (OpenHTMLtoPDF + QR ZXing + métadonnées PDFBox), export en masse asynchrone tous projets (Spring Batch, `ticketsBulkExportJob`, ADMIN uniquement). 115 tests backend (59 rapides + 56 intégration, dont `BulkExportJobIT`/`BulkExportWiringIT`/`ExportWiringIT`), 25 Vitest et 10/10 Playwright inchangés (P14 est backend uniquement). Trois bugs réels trouvés par les tests : entité XML `&middot;` non déclarée rejetée par le rendu XHTML strict d'OpenHTMLtoPDF, `Specification.where(null)` levant une exception dans cette version de Spring Data JPA, et une vraie course entre le statut `COMPLETED` du job (visible avant que le listener de fin de job n'ait fini) et la persistance de l'`ExecutionContext` (`JobRepository.update(...)` ne la persiste jamais, confirmé dans le code source Spring Batch) — corrigée en déplaçant la finalisation vers un `StepExecutionListener.afterStep(...)` avec un appel explicite à `updateExecutionContext(...)`. CI Gate, Backend, SonarQube, CodeQL et Trivy tous vérifiés verts sur GitHub avant fusion. Détail complet ci-dessous. | Aucun pour le critère de sortie de P14. |
| 2026-08-22 | 15 — Registry, release et staging | **Terminée et fusionnée** | PR [#46](https://github.com/Joe-azar/taskira/pull/46), commit de fusion `e2072bc6b2c8f3571d6b5db663b5e698bbaeeb14`, branche `feat/phase15-registry-staging` supprimée après fusion. 4 commits vérifiés indépendamment (`8b9abff`..`e093e84`) : ADR-0013 promu Accepted, `.github/workflows/release.yml` (déclenché sur tag `v*.*.*`, publie GHCR sous version+SHA, jamais `latest` seul), `infra/docker-compose.staging.yml` (déploie une image GHCR déjà publiée, jamais de build local), rollback = redéploiement du même fichier avec un `VERSION` différent. Workflow réellement déclenché deux fois par un vrai tag `v0.1.0` avant l'ouverture de la PR : deux bugs réels trouvés et corrigés au premier essai (variables Compose en `env:` de step au lieu de job, `npm ci` dans `frontend/` cassant la résolution de module de `e2e/playwright/playwright.config.ts`), second essai entièrement vert (GHCR, staging réel, 10/10 Playwright, [Release v0.1.0](https://github.com/Joe-azar/taskira/releases/tag/v0.1.0)). Rollback vérifié séparément en local (pull + redéploiement de `v0.1.0`, stack saine, endpoints confirmés). CI Gate, Backend, SonarQube, CodeQL et Trivy tous vérifiés verts sur GitHub avant fusion. Détail complet ci-dessous. | Aucun pour le critère de sortie de P15. |
| 2026-08-22 | 16 — Sauvegarde et restauration | **Terminée et fusionnée** | PR [#47](https://github.com/Joe-azar/taskira/pull/47), commit de fusion `3eacaaf`, branche `feat/phase16-backup-restore` supprimée après fusion. 5 commits vérifiés indépendamment (`4c9f095`..`2b8f784`) : ADR-0023, `.github/workflows/backup-restore-drill.yml` (hebdomadaire, sème de vraies données via l'API réelle, sauvegarde, restaure, vérifie), correctifs sur les scripts existants de P6. Deux bugs réels trouvés et corrigés avant fusion en exécutant réellement le cycle complet : condition de course avec le serveur PostgreSQL temporaire du point d'entrée officiel de l'image, et extraction du chemin de sauvegarde cassée par un chemin contenant des espaces. Un troisième bug réel trouvé uniquement par le tout premier run réel sur GitHub Actions après fusion (capture `Write-Host` au lieu d'une vraie valeur de retour), corrigé par PR [#48](https://github.com/Joe-azar/taskira/pull/48) (commit de fusion `e03c51f`); second run réel entièrement vert ([32567218685](https://github.com/Joe-azar/taskira/actions/runs/32567218685)). Détail complet ci-dessous. | Aucun pour le critère de sortie de P16. |
| — | 17–20 | Planifiées | Aucun critère de sortie déclaré atteint. | Suivre [la feuille de route](docs/migration-matrix.md) dans l'ordre. |
...
Menée entièrement sur `feat/phase12-notifications`, en six commits vérifiés indépendamment (`2016f11` à `318e4c7`), précédée d'un ADR (`0020`) écrit avant tout code. Fusionnée dans `main` via [PR #35](https://github.com/Joe-azar/taskira/pull/35), commit de fusion `40e791ed30cdd5bb0bb84bbadd7be7a91924907a`, branche supprimée après fusion.
...
## Résultats de la phase 13 (pièces jointes)

Menée entièrement sur `feat/phase13-attachments`, en huit commits vérifiés indépendamment (`817d8fc` à `6104def`), précédée d'un ADR (`0021`) écrit avant tout code et de la promotion d'ADR-0009 (`Proposed` → `Accepted`). Fusionnée dans `main` via [PR #36](https://github.com/Joe-azar/taskira/pull/36), commit de fusion `0e535d2cd94e84fabaa3163859a33f8a48f4604f`, branche supprimée après fusion.

### Module `attachments`

Entité `Attachment` (`AuditableEntity`, liée à `Ticket` et à l'utilisateur uploadeur), table `attachments` (Flyway `V9` : FK `ticket_id` en cascade, FK `uploader_id`, `storage_key` unique, colonnes de version/horodatage standard). Port `DocumentStorage` avec une seule implémentation `LocalFileSystemStorage` — exactement ce qu'annonçait ADR-0009 avant que le module n'existe. Module fermé par défaut (aucune autre partie du code ne le consomme), sans cycle, confirmé par `ModularityTests`.

### Ne jamais faire confiance à l'entrée client

- **Type MIME réel, pas déclaré** : Apache Tika (`tika-core` 3.3.1, version stable actuelle confirmée via l'API de recherche Maven Central — `4.0.0` n'existe qu'en alpha, écartée) détecte le type par analyse du contenu (magic bytes), jamais le `Content-Type` du navigateur ni l'extension du fichier. Liste blanche explicite (images courantes, PDF, texte brut, quelques formats bureautiques); tout le reste est rejeté avec `409 Conflict`.
- **Nom de fichier jamais utilisé comme chemin** : la clé de stockage réelle est un UUID généré côté serveur, jamais dérivé du nom fourni par le client. `LocalFileSystemStorage` valide en plus la forme de la clé par regex et revérifie que le chemin résolu reste sous le répertoire de stockage (`startsWith`) — défense en profondeur, pas une seule barrière. Le nom d'origine n'est conservé que comme métadonnée d'affichage, réinjecté au téléchargement via `Content-Disposition: attachment` (jamais `inline`) avec encodage RFC 6266 (`filename*=UTF-8''...`).
- **Taille limitée** à la fois par Spring (`spring.servlet.multipart.max-file-size`) et par le service.
- **SHA-256 calculé et stocké** pour chaque fichier.

### Deux bugs réels trouvés par les tests, pas supposés

1. **SHA-256 d'un contenu vide** : le hash était initialement calculé en enveloppant le flux passé à `DocumentStorage.store(...)` dans un `DigestInputStream`. `AttachmentServiceTest` mocke `DocumentStorage` — un mock n'exécute jamais la vraie lecture du flux qu'il reçoit — donc le digest restait dans son état initial et produisait silencieusement `e3b0c442...`, le SHA-256 bien connu d'un contenu vide, au lieu du hash réel. Révélé par une assertion qui calcule indépendamment le hash attendu et le compare, pas une assertion qui fait confiance au champ retourné. Corrigé en lisant le fichier une seule fois en mémoire (`file.getBytes()`) et en calculant `MessageDigest.getInstance("SHA-256").digest(...)` directement sur ce tableau d'octets, indépendamment de ce que fait l'adaptateur de stockage.
2. **Ordre de validation exposant une erreur trompeuse** : le service résolvait l'utilisateur courant avant de détecter le type MIME réel. Le test du rejet d'un fichier au contenu non autorisé (un script shell nommé `totally-a-photo.png`) attendait un `ConflictException` mais recevait un `ResourceNotFoundException: Current user not found`, parce que ce test n'avait volontairement pas stubé `userRepository.findById(...)` pour un fichier de toute façon destiné au rejet. Corrigé en déplaçant la détection de type et le rejet par liste blanche avant la résolution de l'utilisateur courant — plus correct aussi bien pour le test que pour la conception : un fichier refusé ne doit pas payer le coût d'une résolution utilisateur inutile.

### Un troisième bug, trouvé uniquement en démarrant la vraie stack

Les deux bugs ci-dessus sont sortis des tests automatisés; celui-ci n'est apparu qu'au premier démarrage réel de `infra/docker-compose.yml` reconstruit avec le nouveau code. `LocalFileSystemStorage` échouait dès le démarrage de l'application : `BeanCreationException` → `AccessDeniedException: /var/lib/taskira`. Cause réelle : le backend non-root introduit en P11 (utilisateur `taskira`, uid 10001) n'a jamais eu de répertoire accessible en écriture à ce chemin, et aucun volume n'y était monté — le `@PostConstruct` de `LocalFileSystemStorage` tentait de créer un répertoire sous `/var/lib`, dont le propriétaire dans l'image de base reste `root`. Corrigé en deux temps dans `backend/Dockerfile` : le répertoire est désormais pré-créé et chowné vers `taskira:taskira` avant `USER taskira`, puis un volume nommé (`taskira_attachments_data` en développement, `taskira_attachments_data_prodlike` en production-like) est monté à ce chemin dans `infra/docker-compose.yml` et `infra/docker-compose.prodlike.yml` — le mécanisme de copy-up de Docker sur un volume nommé (par opposition à un bind mount) copie le contenu et les permissions du répertoire existant de l'image vers le volume à la première utilisation, préservant ainsi la propriété posée au build.

### Vérification

100 tests backend (51 rapides + 49 intégration, contre 83 en fin de P12), dont `AttachmentWiringIT` : upload/téléchargement/suppression réels via HTTP et PostgreSQL (pas `AttachmentService` appelé directement) — un vrai PNG accepté et retéléchargé identique octet pour octet, un script shell déguisé en `.png` rejeté (`409`), un non-membre du projet refusé (`403`) à l'upload comme au téléchargement. `LocalFileSystemStorageTest` couvre en plus, isolément, une tentative de traversée de répertoire (`../../../../etc/passwd`) rejetée sans jamais atteindre le filesystem réel en dehors du répertoire de stockage. 25 Vitest, lint et build Angular inchangés (aucun fichier frontend modifié — P13 est backend uniquement, conformément à la feuille de route). 10/10 Playwright (stack de développement isolée, aucun nouveau scénario — aucune UI d'attachments n'existe encore côté frontend).

Un test préexistant a dû être mis à jour, pas seulement les nouveaux : `ProjectRepositoryIT.flywayAppliesEveryMigrationToPostgreSql` vérifie la liste exacte des migrations Flyway appliquées; corrigé pour inclure `"9"` après l'ajout de `V9__create_attachments.sql`, conformément à la règle de ce dépôt sur la création de nouvelles migrations (jamais modifier une migration déjà appliquée).

Vérifié réellement contre la vraie stack de développement après le correctif de permissions, pas seulement en test automatisé : inscription, création de projet et de ticket via l'API réelle, upload d'un vrai PNG (`201`, SHA-256 correct), téléchargement identique octet pour octet avec les en-têtes attendus (`Content-Disposition: attachment`, `Content-Type: image/png`), rejet réel d'un script shell déguisé en `.png` (`409`, type détecté `application/x-sh`), suppression (`204`) avec confirmation de la disparition de la ligne en base (listing vide) et du fichier physique sur disque (répertoire de stockage vérifié vide dans le conteneur), et les deux événements d'audit (`ATTACHMENT_CREATED`, `ATTACHMENT_DELETED`) correctement enregistrés en base avec l'email de l'acteur et le nom du fichier.

### Un quatrième bug, trouvé uniquement par le vrai run GitHub Actions de la PR

Les trois bugs ci-dessus étaient déjà corrigés avant l'ouverture de la Pull Request; celui-ci n'est apparu que sur les checks GitHub distants (`Backend` et `SonarQube analysis and quality gate`, tous deux rouges), jamais localement. Cause réelle, diagnostiquée en lisant les vrais logs des jobs GitHub Actions (récupérés via un jeton déjà présent dans le Gestionnaire d'identification Windows, retrouvé via `git credential fill` — même mécanisme que celui documenté en phase 3/6, pas un contournement) : `mvn verify` avait toujours été exécuté localement dans un conteneur Maven générique tournant en `root`, qui peut écrire n'importe où sur le filesystem — masquant que la valeur par défaut de `app.attachments.storage-path` (`/var/lib/taskira/attachments`) n'est réellement écrivable que grâce au `chown` explicite posé dans `backend/Dockerfile`, pas par défaut sur n'importe quelle machine. Sur le runner GitHub Actions (utilisateur `runner`, non privilégié), `ProdProfileTest` — le seul test Surefire à démarrer un contexte Spring complet — échouait avec `BeanCreationException` → `AccessDeniedException`, ce qui a aussi fait échouer le job `SonarQube analysis and quality gate`, qui exécute le même `mvn verify` avant de lancer l'analyse.

Corrigé en remplaçant la valeur par défaut par un chemin relatif au répertoire de travail de la JVM (`./data/attachments`) — écrivable sans provisioning particulier partout où l'application démarre directement (CI, poste de développement, tout `*Test`/`*IT` qui charge le contexte complet), pas seulement dans l'image Docker de ce dépôt. `backend/Dockerfile` et les deux fichiers Compose ont été alignés sur `/app/data/attachments` (sous le `WORKDIR` de l'image) pour que le comportement de persistance Docker reste inchangé; seul le couplage jusque-là implicite entre la valeur par défaut de l'application et la disposition de cette image particulière est devenu explicite. `backend/.gitignore` ignore désormais `/data/` pour ce répertoire généré localement par un `mvn test`/`mvn verify` exécuté hors Docker.

Revérifié avant repush : `mvn clean verify` propre (100 tests toujours verts, le répertoire relatif effectivement créé et utilisé), la vraie stack de développement reconstruite et re-smoke-testée de bout en bout au nouveau chemin (`/app/data/attachments`), et un nouveau 10/10 Playwright complet. Une fois repoussé, la PR [#36](https://github.com/Joe-azar/taskira/pull/36) a été suivie en temps réel jusqu'à ce que tous les checks obligatoires passent réellement au vert (`CI Gate`, `Backend`, `SonarQube analysis and quality gate`, `Containers and E2E`, CodeQL, Trivy), puis fusionnée.

## Résultats de la phase 14 (exports)

Menée entièrement sur `feat/phase14-exports`, en quatre commits vérifiés indépendamment (`921d73c` à `a8aa118`), précédée d'un ADR (`0022`) écrit avant tout code. Fusionnée dans `main` via [PR #37](https://github.com/Joe-azar/taskira/pull/37), commit de fusion `2f84d5ba39ab57ef65da868a026aab06be3fa646`, branche supprimée après fusion.

### Trois cas réels, pas une vitrine technologique

Deux exports synchrones, bornés par construction : `GET /api/v1/projects/{id}/tickets/export.xlsx` (Apache POI `poi-ooxml` 5.5.1, un seul projet) et `GET /api/v1/tickets/{id}/export.pdf` (OpenHTMLtoPDF `1.0.10`, un seul ticket, HTML construit directement en Java par text blocks — aucun moteur de templating ajouté pour une seule vue fixe dans un backend sans autre vue serveur). Le rapport PDF embarque un QR code ZXing (`core`/`javase` 3.5.3) encodant l'URL web du ticket, en `data:image/png;base64,...` directement dans le HTML.

Un export en masse asynchrone, seul cas qui justifie réellement Spring Batch (volume non borné, tous projets confondus) : `ticketsBulkExportJob`, réservé aux administrateurs (`@PreAuthorize("hasRole('ADMIN')")`), pagine tous les tickets (`ItemReader` maison réutilisant `TicketRepository.findAll(Specification, Pageable)` pour son `@EntityGraph` déjà existant, sans dépendance `spring-batch-data` supplémentaire) dans un `SXSSFWorkbook` streaming partagé sur toute la durée du job, un onglet par code de projet. Lancé de façon asynchrone (`POST /api/v1/exports/tickets/batch`) via un `TaskExecutorJobLauncher` dédié — le `JobLauncher` par défaut de Spring Boot est synchrone et aurait bloqué la requête HTTP jusqu'à la fin de l'export complet. Statut interrogeable (`GET .../batch/{id}`), résultat téléchargeable une fois `COMPLETED` (`GET .../batch/{id}/download`), stocké via le port `DocumentStorage` déjà défini par `attachments` (P13, désormais exposé via `@NamedInterface`) plutôt qu'un second mécanisme de stockage.

Schéma `BATCH_*` de Spring Batch `6.0.4` (résolu par le BOM Spring Boot 4.1.0, copié verbatim depuis `schema-postgresql.sql` du jar `spring-batch-core` correspondant) créé par Flyway `V10`, jamais par l'initialiseur intégré de Spring Boot (`spring.batch.jdbc.initialize-schema: never`, même règle que `ddl-auto: validate` pour Hibernate ailleurs dans ce dépôt); `spring.batch.job.enabled: false` empêche tout lancement automatique du job au démarrage de l'application.

### Une vraie restructuration de package découverte en vérifiant, pas supposée

Spring Batch 6.0 a déplacé une grande partie de son API : `ItemReader`/`ItemWriter`/`ItemProcessor`/`Chunk`/`ExecutionContext` vivent désormais sous `org.springframework.batch.infrastructure.item`, pas `org.springframework.batch.item` comme dans les versions précédentes. Découvert en téléchargeant et lisant directement les jars/sources réels de `spring-batch-core`/`spring-batch-infrastructure`/`spring-batch-test` 6.0.4 depuis Maven Central avant d'écrire le moindre code d'intégration — plutôt que de supposer une API à partir d'une version antérieure — après quoi une première tentative avec l'ancien chemin de package a immédiatement révélé l'erreur (`package org.springframework.batch.item does not exist`), corrigée avant même le premier `mvn compile`.

### Deux bugs réels trouvés à la compilation puis aux tests, pas supposés

1. **Entité XML non déclarée** : `&middot;` (entité HTML nommée) fait échouer le rendu OpenHTMLtoPDF, qui exige du XHTML strict — seules `amp`/`lt`/`gt`/`quot`/`apos` sont valides sans DTD déclarée. Révélé par `TicketPdfExportServiceTest` avec `XRRuntimeException: The entity "middot" was referenced, but not declared`. Corrigé avec la référence numérique `&#183;`.
2. **`Specification.where(null)` lève une exception** : cette version de Spring Data JPA rejette un argument `null` avec `IllegalArgumentException: Specification must not be null` plutôt que de le traiter comme « aucune restriction » — contrairement à l'idiome habituel dans les versions précédentes. Révélé par `TicketPagingItemReaderTest`. Corrigé avec une spécification explicitement toujours vraie (`(root, query, cb) -> cb.conjunction()`), qui réutilise au passage le même `@EntityGraph(project, creator, assignee)` déjà défini sur `TicketRepository.findAll(Specification, Pageable)`.

### Un troisième bug, plus sérieux : une vraie course entre le statut du job et la persistance de son résultat

Trouvé par `BulkExportJobIT`, pas supposé : le job passait bien à `COMPLETED`, mais relire son `ExecutionContext` juste après échouait avec `ClassCastException: Value for key=[storageKey] is not of type: [class java.lang.String], it is [null]` — la clé n'existait tout simplement pas. Cause réelle, diagnostiquée en lisant directement le code source de `AbstractJob`/`SimpleJobRepository` dans Spring Batch 6.0.4 : `AbstractJob.execute()` positionne et persiste le statut terminal du job (`jobRepository.update(execution)`) *avant* d'appeler les `JobExecutionListener`, donc un appelant qui ne sonde que le statut peut observer `COMPLETED` avant même que le listener n'ait commencé à s'exécuter. Pire : `SimpleJobRepository.update(JobExecution)` — le seul appel de persistance qu'`AbstractJob` effectue autour de la complétion — n'appelle que le DAO d'exécution de job, jamais celui de l'`ExecutionContext`; seule une méthode séparée, `updateExecutionContext(JobExecution)`, s'en charge, et rien dans le framework ne l'appelle automatiquement après un listener.

La logique de finalisation (sérialiser le classeur, le stocker, écrire la clé de stockage dans l'`ExecutionContext`, auditer) vivait initialement dans un `JobExecutionListener.afterJob(...)`. Corrigé en la déplaçant vers un `StepExecutionListener.afterStep(...)` sur l'unique step du job — qui s'exécute dans le cadre de la complétion du step lui-même, strictement avant que le job ne détermine et persiste son statut global — avec un appel explicite à `jobRepository.updateExecutionContext(jobExecution)`. `StepBuilder.chunk(int)` de Spring Batch 6.0 (nouvelle signature sans gestionnaire de transaction) a par ailleurs révélé, en lisant son code source, qu'elle bascule silencieusement vers un `ResourcelessTransactionManager` (no-op) si aucun n'est fourni explicitement; corrigé en câblant le vrai `PlatformTransactionManager` JPA de l'application.

### Vérification

115 tests backend (59 rapides + 56 intégration, contre 100 en fin de P13). `TicketExcelExportServiceTest`/`TicketPdfExportServiceTest` relisent les octets réellement produits avec POI/PDFBox et redécodent le QR code avec ZXing depuis la page PDF rastérisée — pas seulement qu'un fichier non vide a été renvoyé. `BulkExportJobIT` lance le job à travers le vrai `JobLauncher` asynchrone de l'application (pas un lanceur synchrone de test) et attend un vrai `BatchStatus.COMPLETED` avant de relire le classeur stocké. `BulkExportWiringIT`/`ExportWiringIT` couvrent le flux HTTP complet (lancement, sondage, téléchargement, contrôle d'accès ADMIN) et l'événement d'audit `EXPORT_GENERATED`. 25 Vitest, lint et build Angular inchangés (aucun fichier frontend modifié — P14 est backend uniquement, comme P12 et P13). 10/10 Playwright (stack de développement isolée, aucun nouveau scénario).

Vérifié réellement contre la vraie stack de développement reconstruite après ce correctif, pas seulement en test automatisé : export Excel synchrone téléchargé et confirmé comme un vrai fichier Microsoft Excel 2007+, export PDF synchrone confirmé comme un vrai PDF 1 page, connexion de l'admin dev bootstrapé, lancement du job en masse (`202`, `jobExecutionId`), sondage jusqu'à `COMPLETED` (quasi instantané sur ce volume de données réel), classeur téléchargé dont les onglets couvrent bien tous les projets existants de la base de développement — y compris ceux créés par d'autres sessions de test antérieures dans cette même conversation — événement d'audit `EXPORT_GENERATED` confirmé directement en base avec l'email de l'administrateur, et lancement refusé (`403`) pour un utilisateur non-admin.

## Résultats de la phase 15 (registry, release et staging)

Menée entièrement sur `feat/phase15-registry-staging`, précédée de la promotion d'ADR-0013 (`Proposed` → `Accepted`, écrite avant le workflow lui-même). Fusionnée dans `main` via [PR #46](https://github.com/Joe-azar/taskira/pull/46), commit de fusion `e2072bc6b2c8f3571d6b5db663b5e698bbaeeb14`, branche supprimée après fusion.

### Publication GHCR et vérification de déploiement avant release formelle

`.github/workflows/release.yml` se déclenche sur `push` d'un tag `v*.*.*` : construit et pousse `ghcr.io/joe-azar/taskira-backend`/`taskira-frontend` sous deux tags à chaque fois (la version et le SHA du commit — jamais `latest` comme seule référence, conformément à ADR-0013), puis déploie réellement la version tout juste publiée via `infra/docker-compose.staging.yml` — le seul fichier Compose de ce dépôt qui ne construit jamais d'image localement, chaque service référence `image: ghcr.io/...:${VERSION:?...}` — et exécute la suite Playwright complète (`e2e/playwright/tests/`) contre ce déploiement réel avant de publier une `GitHub Release` formelle (notes auto-générées via `gh release create --generate-notes`). La release n'apparaît que si le déploiement et les tests ont réellement réussi, jamais sur la seule force d'un build vert.

`infra/docker-compose.staging.yml` reprend la topologie à trois réseaux de `docker-compose.prodlike.yml` (P11), moins Prometheus/Grafana (déjà validés en P10/P11; staging se concentre sur la vérification de déploiement, pas sa duplication). Première version réellement publiée : `v0.1.0`, pas `v1.0.0` — SemVer `0.x` signale explicitement « pré-1.0, en évolution », cohérent avec le fait que les phases 15 à 20 ne sont pas terminées; le numéro de version n'est délibérément pas indexé sur le numéro de phase.

### Deux bugs réels trouvés par le premier run réel déclenché par tag

Le workflow a été déclenché réellement en poussant un vrai tag `v0.1.0` — pas seulement validé par `actionlint`/`docker compose config` en local — révélant deux bugs qu'aucune validation statique n'aurait trouvés :

1. **Variables Compose en `env:` de step, pas de job** : `VERSION`/`POSTGRES_PASSWORD`/`FRONTEND_HTTP_PORT` n'étaient définies que sur l'étape `up`. Chaque invocation `docker compose -f docker-compose.staging.yml` réinterpole tout le fichier quelle que soit la sous-commande; l'étape de démontage (`down`) échouait avec la même erreur `POSTGRES_PASSWORD must be set` que le fichier est censé lever volontairement en cas d'absence réelle — sauf qu'ici la stack tournait déjà et n'a jamais été nettoyée par cette étape. Corrigé en déplaçant ces variables au niveau du job (`jobs.staging-smoke-test.env`), héritées par toutes les étapes.
2. **`npm ci` exécuté dans `frontend/`** : `e2e/playwright/playwright.config.ts` résout son propre `import ... from '@playwright/test'` relativement à l'emplacement du fichier de configuration lui-même (un répertoire frère de `frontend/`, jamais un ancêtre) — pas relativement au répertoire depuis lequel `npx` est invoqué. `Cannot find module '@playwright/test'` malgré une résolution `npx` elle-même parfaitement correcte (le CLI Playwright a bien été trouvé). Corrigé en installant les dépendances à la racine du dépôt, reproduisant exactement l'approche déjà utilisée par `e2e/playwright/Dockerfile` (qui copie `frontend/package.json`/`package-lock.json` vers `/workspace` avant `npm ci`, précisément pour cette raison).

### Second run réel : entièrement vert

Après correction, le tag `v0.1.0` a été supprimé et recréé sur le commit corrigé (aucune `GitHub Release` n'avait été publiée pour la première tentative — `publish-release` dépend de `staging-smoke-test` et ne s'exécute jamais si ce dernier échoue — donc rien à révoquer, seulement un tag Git à redéplacer). Résultat intégralement vert : images construites et poussées sur GHCR, stack staging réellement démarrée et saine (healthchecks backend et frontend confirmés), 10/10 Playwright contre ce déploiement réel, [`GitHub Release v0.1.0`](https://github.com/Joe-azar/taskira/releases/tag/v0.1.0) publiée avec notes auto-générées couvrant l'historique complet des Pull Requests fusionnées.

### Rollback vérifié réellement, indépendamment du workflow CI

Le mécanisme de rollback n'est jamais qu'un redéploiement de `infra/docker-compose.staging.yml` avec un `VERSION` différent — le même chemin de code que le workflow de release emprunte déjà pour son propre déploiement de vérification, pas une procédure séparée et non testée. Vérifié concrètement en local, hors du run CI : `docker pull ghcr.io/joe-azar/taskira-{backend,frontend}:v0.1.0` (téléchargement réel confirmé par digest), puis déploiement local via `docker-compose.staging.yml` avec ce `VERSION` — stack saine (`healthy` sur les trois services), `GET /api/v1/auth/me` répondant `401` à travers le proxy Nginx (comportement anonyme attendu), `GET /healthz` `200`, page SPA `200` — avant démontage complet (`down --volumes --remove-orphans`) et suppression du fichier de secrets locaux.

### Vérification

Validé localement avant tout push : `actionlint 1.7.12` sur l'ensemble des workflows (0 problème après correction d'un avertissement shellcheck cosmétique), `docker compose config` sur `docker-compose.staging.yml` (cas valide avec `VERSION`/`POSTGRES_PASSWORD` fournis, et cas de rejet explicite lorsqu'ils sont absents). Le workflow réel a ensuite été déclenché deux fois par un vrai tag, pas seulement supposé fonctionnel après validation statique — voir ci-dessus pour les deux bugs trouvés et corrigés entre les deux essais.

## Résultats de la phase 16 (sauvegarde et restauration testée)

Menée entièrement sur `feat/phase16-backup-restore`, précédée d'un ADR (`0023`) écrit avant le workflow lui-même. Fusionnée dans `main` via [PR #47](https://github.com/Joe-azar/taskira/pull/47), commit de fusion `3eacaaf`, branche supprimée après fusion.

### Deux mécanismes distincts, pas un seul mal défini

P6 avait déjà produit `scripts/backup/backup-postgres.ps1` et `scripts/restore/restore-postgres.ps1` (utilisés pour la migration PostgreSQL 16 → 18, voir [ADR-0017](docs/adr/0017-postgresql-18-migration.md)) — la sauvegarde/restauration à la demande. Ce qui manquait : une politique récurrente. Contrainte réelle assumée et documentée dans l'ADR plutôt qu'ignorée : ce dépôt n'a, à ce stade, aucune base de données de production persistante et accessible en continu — le staging de P15 est déployé et détruit à la demande, la vraie production reste hors périmètre avant P19. Un job planifié ne peut donc pas « sauvegarder la production »; son rôle réel est de prouver en continu que le mécanisme de sauvegarde/restauration ne se dégrade pas silencieusement.

`.github/workflows/backup-restore-drill.yml` (hebdomadaire, `workflow_dispatch` également disponible) démarre un vrai PostgreSQL et le vrai backend (toutes les migrations Flyway réellement appliquées), sème de vraies données via l'API HTTP réelle (un utilisateur, un projet, un ticket), sauvegarde, restaure dans un conteneur jetable, puis **vérifie** que les données semées sont réellement revenues — pas seulement que `pg_restore` a rendu un code de sortie zéro. Réutilise directement les scripts existants via `shell: pwsh` (même idiome déjà établi pour `scripts/sonarqube/bootstrap.ps1` dans `quality.yml`), pas une seconde implémentation en bash à maintenir séparément.

### Deux bugs réels trouvés en exécutant réellement le cycle complet, pas supposés

1. **Condition de course avec le serveur PostgreSQL temporaire** : un vrai cycle sauvegarde → restauration contre la base de développement réelle (10 migrations, données accumulées réelles) a révélé que `restore-postgres.ps1` échouait avec `FATAL: the database system is shutting down`. Cause réelle : le point d'entrée officiel de l'image PostgreSQL démarre un serveur *temporaire* (socket Unix seulement) pour exécuter les scripts d'initialisation, l'arrête, puis démarre le serveur réel — `pg_isready` seul pouvait réussir contre ce serveur temporaire, juste avant son arrêt. Corrigé en remplaçant la vérification par une boucle de nouvelle tentative autour d'une vraie requête `psql -c "SELECT 1"` contre la base cible, qui traverse naturellement cette fenêtre transitoire au lieu d'entrer en course avec elle. Un correctif associé a aussi retiré une redirection `stderr` : PowerShell 5.1 la transforme en erreur bloquante sous `$ErrorActionPreference = "Stop"`, ce qui interrompait la boucle de nouvelle tentative dès son premier échec attendu.
2. **Extraction du chemin de sauvegarde cassée par un chemin contenant des espaces** : `workflow_dispatch` ne peut pas déclencher un tout nouveau fichier de workflow avant qu'il n'existe sur la branche par défaut — contrairement au workflow de release de P15 (déclenché par un tag, testable avant fusion), ce workflow planifié n'a pas pu être exécuté réellement sur GitHub avant l'ouverture de la PR. Les étapes ont donc été rejouées à la main en local à la place, révélant qu'une regex `\S+` utilisée pour extraire le chemin de sauvegarde depuis la sortie du script s'arrêtait au premier espace — et le chemin de ce dépôt lui-même (« ...ALL DATA\France\Taskira... ») en contient un. Corrigé avec une regex gourmande ancrée sur le suffixe fixe du message (« (N bytes). »), qui revient correctement en arrière quel que soit le contenu du chemin.

### Vérification avant fusion

Cycle complet exécuté réellement en local avant toute ouverture de PR, pas seulement validé statiquement : semer de vraies données via l'API réelle (utilisateur, projet, ticket), sauvegarder la base de développement réelle (10 migrations, données accumulées de toute cette session de migration), restaurer dans un conteneur jetable, puis exécuter les requêtes de vérification exactes utilisées par le workflow (`SELECT count(*)` sur `users`/`projects`/`tickets`) — toutes confirmant que les données semées sont bien revenues. `actionlint 1.7.12` sur l'ensemble des workflows (0 problème après correction d'une collision de syntaxe YAML : l'opérateur d'appel PowerShell `&` en tête de ligne d'un scalaire à une seule ligne est interprété comme une ancre YAML).

### Un troisième bug réel, trouvé uniquement par le tout premier run réel sur GitHub Actions

`workflow_dispatch` ne pouvant pas déclencher un workflow inexistant sur la branche par défaut, le tout premier run réel de `backup-restore-drill.yml` n'a pu avoir lieu qu'après la fusion de la PR #47 — la répétition locale décrite ci-dessus, aussi rigoureuse soit-elle, ne pouvait pas se substituer entièrement à une vraie exécution sur le runner GitHub Actions. Ce premier run ([32566783030](https://github.com/Joe-azar/taskira/actions/runs/32566783030)) a effectivement échoué, à l'étape « Back up the seeded database » : `Cannot index into a null array`.

Cause réelle, distincte des deux bugs déjà trouvés en local : `backup-postgres.ps1` ne rapporte son chemin de sauvegarde que via `Write-Host`, qui écrit directement sur la console et n'atteint jamais le flux de sortie de succès de PowerShell — `$result = & backup-postgres.ps1` capturait donc systématiquement `$null`. La répétition locale avant fusion avait testé la regex d'extraction contre un texte synthétique reproduisant le format attendu, pas contre cette sémantique de capture réelle du script; c'est cette différence précise qui a laissé le bug passer inaperçu jusqu'au vrai run.

Corrigé à la racine plutôt que par une nouvelle regex : `backup-postgres.ps1` se termine désormais par une expression nue (`$hostPath`), qui devient sa vraie valeur de retour sur le flux de succès, capturable directement (`$dumpPath = & backup-postgres.ps1 | Select-Object -Last 1`) sans plus jamais analyser du texte de log — les lignes `Write-Host` existantes restent inchangées pour l'usage interactif documenté dans `docs/architecture/backup.md`. Corrigé sur une branche de suivi dédiée `fix/backup-drill-real-capture` ([PR #48](https://github.com/Joe-azar/taskira/pull/48), commit de fusion `e03c51f`), re-vérifié réellement contre la vraie stack de développement avant push (cycle complet sauvegarde/restauration, chemin contenant un espace correctement capturé, 15 utilisateurs/7 projets/5 tickets confirmés après restauration), CI Gate et tous les checks obligatoires vérifiés verts avant fusion.

Un second run réel du workflow, déclenché après ce correctif, est passé intégralement au vert ([32567218685](https://github.com/Joe-azar/taskira/actions/runs/32567218685)) : sème, sauvegarde, restaure, vérifie — toutes les étapes réussies sur le vrai runner GitHub Actions, confirmant le mécanisme au-delà de la seule répétition locale.
...
3. Exercice de sauvegarde/restauration hebdomadaire terminé et fusionné P16 (`.github/workflows/backup-restore-drill.yml`, PR #47/#48), vérifié réellement vert sur GitHub Actions; il prouve que le mécanisme fonctionne toujours, mais aucune base de données de production persistante n'existe encore à sauvegarder réellement (P19+, voir [ADR-0023](docs/adr/0023-backup-restore-strategy.md)); `backups/` local n'a aujourd'hui aucune politique de rétention/rotation automatique.
...
8. Aucune UI frontend pour les pièces jointes (P13 est backend uniquement, conformément à la feuille de route); l'API existe et est testée mais rien ne la consomme encore dans Angular. ClamAV reste optionnel, non ajouté dans cette première version (décision assumée, voir [ADR-0021](docs/adr/0021-attachments-storage.md)); aucun quota par projet/utilisateur.
9. Aucune UI frontend pour les exports (P14 est backend uniquement, même logique que P12/P13); les trois endpoints existent et sont testés mais rien ne les consomme encore dans Angular. Le classeur en masse n'est jamais purgé automatiquement du stockage après téléchargement (aucune politique de rétention) et le `ThreadPoolTaskExecutor` dédié au lancement du job est en mémoire de l'instance applicative — un redémarrage pendant un job en cours abandonnerait son exécution — décisions assumées pour ce cas d'usage rare et manuel, voir [ADR-0022](docs/adr/0022-exports-batch.md).
