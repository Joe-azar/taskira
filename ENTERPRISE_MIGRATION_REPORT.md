...
| 2026-08-22 | 15 — Registry, release et staging | Terminée localement | Branche `feat/phase15-registry-staging`, non fusionnée. 3 commits vérifiés indépendamment (`8b9abff`..`3f7433f`) : ADR-0013 promu Accepted, `.github/workflows/release.yml` (déclenché sur tag `v*.*.*`, publie GHCR sous version+SHA, jamais `latest` seul), `infra/docker-compose.staging.yml` (déploie une image GHCR déjà publiée, jamais de build local), rollback = redéploiement du même fichier avec un `VERSION` différent. Workflow réellement déclenché deux fois par un vrai tag `v0.1.0` : deux bugs réels trouvés et corrigés au premier essai (variables Compose en `env:` de step au lieu de job, `npm ci` dans `frontend/` cassant la résolution de module de `e2e/playwright/playwright.config.ts`), second essai entièrement vert (GHCR, staging réel, 10/10 Playwright, [Release v0.1.0](https://github.com/Joe-azar/taskira/releases/tag/v0.1.0)). Rollback vérifié séparément en local (pull + redéploiement de `v0.1.0`, stack saine, endpoints confirmés). Détail complet ci-dessous. | PR à ouvrir/merger par le propriétaire (`gh` non authentifié dans cet environnement). |
...
Menée entièrement sur `feat/phase15-registry-staging`, précédée de la promotion d'ADR-0013 (`Proposed` → `Accepted`, écrite avant le workflow lui-même).
