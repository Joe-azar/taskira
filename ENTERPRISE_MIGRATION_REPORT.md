...
Les phases 0 à 12 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 13 (pièces jointes, Tika, stockage local) est terminée localement sur la branche dédiée `feat/phase13-attachments` (100 tests backend, 25 Vitest, 10/10 Playwright, plus un smoke test manuel réel contre la vraie stack de développement) mais pas encore fusionnée : PR à ouvrir/merger par le propriétaire du dépôt (`gh` non authentifié dans cet environnement). Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`, `feat/phase12-notifications`, `feat/phase13-attachments`), jamais directement sur `main`. Les phases 14 à 20 ne sont pas déclarées terminées.
...
| 2026-08-16 | 13 — Pièces jointes | Terminée localement | Branche `feat/phase13-attachments`, non fusionnée. 6 commits vérifiés indépendamment (`817d8fc`..`a44ab2c`) : ADR-0021 (ADR-0009 promu Accepted), module `attachments` (`Attachment`, Flyway `V9`), port `DocumentStorage`/`LocalFileSystemStorage`, détection de type réel par Apache Tika avec liste blanche, clé de stockage UUID jamais dérivée du client, SHA-256, endpoints upload/téléchargement/suppression audités. 100 tests backend (51 rapides + 49 intégration, dont `AttachmentWiringIT`), 25 Vitest et 10/10 Playwright inchangés (P13 est backend uniquement). Deux bugs réels trouvés par les tests (SHA-256 d'un flux jamais lu par un mock, ordre de validation exposant une erreur trompeuse) et un troisième trouvé uniquement en démarrant la vraie stack (répertoire de stockage non accessible en écriture au backend non-root de P11), tous corrigés et revérifiés. Détail complet ci-dessous. | PR à ouvrir/merger par le propriétaire (`gh` non authentifié dans cet environnement). |
...
Menée entièrement sur `feat/phase13-attachments`, en six commits vérifiés indépendamment (`817d8fc` à `a44ab2c`), précédée d'un ADR (`0021`) écrit avant tout code et de la promotion d'ADR-0009 (`Proposed` → `Accepted`). Terminée localement, pas encore fusionnée : la PR reste à ouvrir/merger par le propriétaire du dépôt (`gh` non authentifié dans cet environnement, comme à chaque phase précédente).
