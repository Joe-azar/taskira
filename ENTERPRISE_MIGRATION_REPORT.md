...
Les phases 0 à 10 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 9 (module `audit`/`audit_events`, request/correlation ID, MDC, logs structurés) a été fusionnée dans `main` le 16 août 2026 (PR #32, commit de fusion `1e6170b`), vérifiée avant fusion (65 tests backend, 25 Vitest, 10/10 Playwright) sur la branche dédiée `feat/phase9-audit-logging`, supprimée après fusion. La phase 10 (Actuator, Micrometer, Prometheus, Grafana) a été fusionnée dans `main` le 16 août 2026 (PR #33, commit de fusion `8f560fe110f71e211ff5db14dc9dbcb4cb0337b6`), vérifiée avant fusion (76 tests backend, 25 Vitest, 10/10 Playwright) sur la branche dédiée `feat/phase10-observability`, supprimée après fusion. La phase 11 (runtime production-like Nginx/non-root) est terminée localement sur `feat/phase11-production-runtime` (76 tests backend, 25 Vitest, 10/10 Playwright, plus un smoke test Playwright réel contre la stack production-like démarrée pour de vrai), pas encore fusionnée — la PR n'est pas ouverte (`gh` n'est pas authentifié dans cet environnement). Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`), jamais directement sur `main`. Les phases 12 à 20 ne sont pas déclarées terminées.
...
Menée entièrement sur `feat/phase11-production-runtime`, en cinq commits vérifiés indépendamment (`2c1b051` à `736e5f0`), précédée d'un ADR (`0019`) écrit avant tout code pour figer les décisions de conception. PR pas encore ouverte : `gh` n'est pas authentifié dans cet environnement. Cette phase a trouvé un nombre inhabituellement élevé de bugs réels — précisément parce que chaque décision a été vérifiée en démarrant réellement des conteneurs plutôt qu'en se fiant à `docker compose config` ou à une lecture de la documentation.
...
Nginx, frontend statique de production et utilisateur non-root sont terminés localement en P11 (`infra/docker-compose.prodlike.yml`, détail dans la section dédiée ci-dessus), pas encore fusionnés dans `main`. GHCR, staging et rollback restent planifiés en P15.
...
2. Image frontend production/Nginx terminée localement en P11, pas encore fusionnée dans `main`; staging reste planifié en P15.
