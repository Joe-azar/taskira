...
Les phases 0 à 14 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 14 (exports Excel/PDF/QR, Spring Batch) a été fusionnée dans `main` le 16 août 2026 (PR [#37](https://github.com/Joe-azar/taskira/pull/37), commit de fusion `2f84d5ba39ab57ef65da868a026aab06be3fa646`) sur la branche dédiée `feat/phase14-exports`, supprimée après fusion — tous les checks obligatoires (`CI Gate`, `Backend`, `SonarQube analysis and quality gate`, `Containers and E2E`, CodeQL, Trivy) vérifiés verts sur GitHub avant fusion. Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`, `feat/phase12-notifications`, `feat/phase13-attachments`, `feat/phase14-exports`), jamais directement sur `main`. Les phases 15 à 20 ne sont pas déclarées terminées.
