...
Les phases 0 à 15 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 15 (registry GHCR, release, staging, rollback) a été fusionnée dans `main` le 22 août 2026 (PR [#46](https://github.com/Joe-azar/taskira/pull/46), commit de fusion `e2072bc6b2c8f3571d6b5db663b5e698bbaeeb14`) sur la branche dédiée `feat/phase15-registry-staging`, supprimée après fusion. Son workflow de release avait déjà été déclenché et vérifié réellement à deux reprises par un vrai tag `v0.1.0` avant même l'ouverture de la PR (deux bugs réels trouvés et corrigés au premier essai, second essai entièrement vert : images publiées sur GHCR, staging réellement déployé, 10/10 Playwright contre ce déploiement réel, [GitHub Release v0.1.0](https://github.com/Joe-azar/taskira/releases/tag/v0.1.0) publiée) — détail complet dans la section dédiée ci-dessous. La phase 16 (sauvegarde et restauration testée) est terminée localement sur la branche dédiée `feat/phase16-backup-restore` mais pas encore fusionnée : PR à ouvrir/merger par le propriétaire du dépôt. Un cycle complet sauvegarde/restauration a déjà été vérifié réellement en local contre la base de développement (10 migrations, données accumulées réelles), révélant et corrigeant deux bugs réels — détail complet dans la section dédiée ci-dessous. Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`, `feat/phase11-production-runtime`, `feat/phase12-notifications`, `feat/phase13-attachments`, `feat/phase14-exports`, `feat/phase15-registry-staging`, `feat/phase16-backup-restore`), jamais directement sur `main`. Les phases 17 à 20 ne sont pas déclarées terminées.
...
| 2026-08-22 | 16 — Sauvegarde et restauration | Terminée localement | Branche `feat/phase16-backup-restore`, non fusionnée. 3 commits vérifiés indépendamment (`4c9f095`..`e01e72a`) : ADR-0023, `.github/workflows/backup-restore-drill.yml` (hebdomadaire, sème de vraies données via l'API réelle, sauvegarde, restaure, vérifie), correctifs sur les scripts existants de P6. Deux bugs réels trouvés et corrigés en exécutant réellement le cycle complet : condition de course avec le serveur PostgreSQL temporaire du point d'entrée officiel de l'image (`FATAL: the database system is shutting down` malgré un `pg_isready` réussi), et extraction du chemin de sauvegarde cassée par un chemin contenant des espaces (celui de ce dépôt lui-même). Cycle complet vérifié réellement en local contre la base de développement (10 migrations, données accumulées réelles). Détail complet ci-dessous. | PR à ouvrir/merger par le propriétaire (`gh` non authentifié dans cet environnement). |
...
Menée entièrement sur `feat/phase16-backup-restore`, précédée d'un ADR (`0023`) écrit avant le workflow lui-même.
...
3. Exercice de sauvegarde/restauration hebdomadaire terminé localement P16 (`.github/workflows/backup-restore-drill.yml`), pas encore fusionné; il prouve que le mécanisme fonctionne toujours, mais aucune base de données de production persistante n'existe encore à sauvegarder réellement (P19+, voir [ADR-0023](docs/adr/0023-backup-restore-strategy.md)); `backups/` local n'a aujourd'hui aucune politique de rétention/rotation automatique.
