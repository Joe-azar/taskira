...
Les phases 0 à 8 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 9 (module `audit`/`audit_events`, request/correlation ID, MDC, logs structurés) est terminée et vérifiée (65 tests backend, 25 Vitest, 10/10 Playwright) sur la branche dédiée `feat/phase9-audit-logging`, pas encore fusionnée dans `main` au moment de la rédaction de cette section. Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`), jamais directement sur `main`. Les phases 10 à 20 ne sont pas déclarées terminées.
...
| 2026-08-16 | 9 — Audit, request ID et logs | Terminée localement | Branche `feat/phase9-audit-logging`, 7 commits atomiques vérifiés indépendamment : module `audit` (`audit_events`, Flyway `V8`), `AuditService.record(...)` en transaction indépendante (`REQUIRES_NEW`), déclencheurs sur connexion/déconnexion/création utilisateur/ticket/projet/membres, `RequestIdFilter` (premier de la chaîne de sécurité), MDC, `ProblemDetails.of(...)` unifiant 3 sites de construction d'erreur, logs structurés JSON en `prod`, endpoint admin `GET /api/v1/audit/events`. 65 tests backend (29 rapides + 36 intégration), 25 Vitest, 10/10 Playwright. Détail complet ci-dessous. | PR à ouvrir, checks GitHub à vérifier, puis fusion dans `main`. |
| — | 10–20 | Planifiées | Aucun critère de sortie déclaré atteint. | Suivre [la feuille de route](docs/migration-matrix.md) dans l'ordre. |
...
3. Observabilité partielle : corrélation de requêtes et logs structurés terminés en P9, mais aucune métrique (Actuator/Micrometer/Prometheus/Grafana, P10) ni stratégie backup/restore planifiée récurrente (le script ponctuel de P6 couvre la sauvegarde/restauration à la demande, pas une politique quotidienne/hebdomadaire/mensuelle).
