...
Les phases 0 à 9 sont terminées et fusionnées (phase 5 pour son critère mécanique, dette architecturale résiduelle documentée). `main` est protégée depuis le 15 août 2026 (`protected: true` vérifié via l'API GitHub). La phase 9 (module `audit`/`audit_events`, request/correlation ID, MDC, logs structurés) a été fusionnée dans `main` le 16 août 2026 (PR #32, commit de fusion `1e6170b`), vérifiée avant fusion (65 tests backend, 25 Vitest, 10/10 Playwright) sur la branche dédiée `feat/phase9-audit-logging`, supprimée après fusion. La phase 10 (Actuator, Micrometer, Prometheus, Grafana) est terminée localement sur `feat/phase10-observability` (76 tests backend, 25 Vitest, 10/10 Playwright), pas encore fusionnée — la PR n'est pas ouverte (`gh` n'est pas authentifié dans cet environnement). Toute la migration a été menée sur des branches dédiées (`feat/phase6-stack-upgrade`, `chore/sync-enterprise-migration-status`, `feat/phase7-api-concurrency`, `feat/phase8-session-security`, `feat/phase9-audit-logging`, `feat/phase10-observability`), jamais directement sur `main`. Les phases 11 à 20 ne sont pas déclarées terminées.
...
| 2026-08-16 | 10 — Observabilité | Terminée localement | Branche `feat/phase10-observability`, 8 commits atomiques vérifiés indépendamment : Actuator/Micrometer sur port de gestion isolé (`EndpointRequest.toAnyEndpoint().permitAll()`), agrégats `countByStatus`, `BusinessMetricsBinder` (jauges tickets/projets/utilisateurs) et compteur de tentatives de connexion, healthcheck Compose, Prometheus (`v3.13.2`)/Grafana (`13.1.3`) provisionnés et épinglés par digest. 76 tests backend (33 rapides + 43 intégration), 25 Vitest (inchangé), 10/10 Playwright (rejeu isolé après un premier run faussé par une exécution Docker concurrente). Détail complet ci-dessous. | PR pas encore ouverte (`gh` non authentifié dans cet environnement) — voir critère de sortie de P10 une fois fusionnée. |
| — | 11–20 | Planifiées | Aucun critère de sortie déclaré atteint. | Suivre [la feuille de route](docs/migration-matrix.md) dans l'ordre. |
...
Menée entièrement sur `feat/phase10-observability`, en huit commits atomiques (`5c171f3` à `32900ae`), chacun vérifié (build + tests réels) avant le suivant. PR pas encore ouverte : `gh` n'est pas authentifié dans cet environnement.
...
Actuator, Micrometer, Prometheus et Grafana sont terminés localement en P10 (détail dans la section dédiée ci-dessus), pas encore fusionnés dans `main`. Les request IDs, MDC et logs structurés appartiennent à P9 (fusionnée). Loki reste optionnel en P20.
...
3. Métriques (Actuator/Micrometer/Prometheus/Grafana) terminées localement en P10 mais pas encore fusionnées dans `main`; aucune stratégie backup/restore planifiée récurrente (le script ponctuel de P6 couvre la sauvegarde/restauration à la demande, pas une politique quotidienne/hebdomadaire/mensuelle, planifiée en P16).
